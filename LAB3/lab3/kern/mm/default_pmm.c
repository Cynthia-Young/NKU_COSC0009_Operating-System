...
/*  In the First Fit algorithm(最先匹配), the allocator(分配器) keeps a list of free blocks
 ...
 * is usually splitted(分割), and the remainder(余数) will be added into the list as
 * another free block.
 ...
 * Details of FFMA
 * (1) Preparation:
 *  In order to implement(使生效) the First-Fit Memory Allocation (FFMA), we should
 ...
 * `list_next`, `list_prev`.
 *  There's a tricky（棘手的） method that is to transform a general `list` struct to a
 ...
 */
free_area_t free_area;//通过双向链表来完成对空闲块的管理

#define free_list (free_area.free_list)//链表头是free_area.free_list，链表项是Page数据结构的base->page_link
...
/*firstfit算法的改进
在进行分配以及释放内存的时候，在双向链表上进行操作的时间复杂度为O(n)，如果使用二叉搜索树对地址进行排序，
从而对进程进行管理，就可以在查找页块时将时间复杂度降到O(logn)；*/ 
